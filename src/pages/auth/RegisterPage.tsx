...
import { Eye, EyeOff, Mail, Lock, User, AlertCircle, CheckCircle2, ArrowRight } from 'lucide-react';
...
export const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const { error, needsEmailConfirmation } = await signUp(email.trim(), password, displayName.trim());
    setIsLoading(false);
  ...
  const handleLineSignIn = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to register with LINE.';
      setErrorMessage(msg);
    } finally {
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-gradient-to-b from-surface via-primary-light/30 to-surface">
      <div className="w-full max-w-md space-y-6">
        <Card className="p-8 sm:p-10 shadow-card border-primary/10">
          ...
          <div className="text-center space-y-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-text-secondary mt-1">
                Join Word Buddy & start leveling up!
              </p>
            </div>
          </div>
          ...
          {successMessage && (
            <div className="mb-5 p-3.5 rounded-xl bg-accent-green-light border border-accent-green/20 flex items-start gap-2.5 text-accent-emerald text-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input
              ...
              type="text"
              placeholder="e.g. Nida Jitprasong"
              value={displayName}
